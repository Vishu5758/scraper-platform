...
def run_pipeline(
    ...
    try:
        ...
        registry = ComponentRegistry.from_yaml(components_yaml)
        ...
        results = engine.execute(compiled, runtime_params=runtime_params)

        # Extract item count from results if available
        item_count = None
        if isinstance(results, dict):
            item_count = results.get("item_count") or results.get("records_count")
